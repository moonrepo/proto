...
impl AsdfPlugin {
    ...
    pub fn download_prebuilt(&self, input: DownloadPrebuiltInput) -> DownloadPrebuiltOutput {
        ...
        check_supported_os_and_arch(
            ...
            permutations![
                ...
                // Aren't supported for windows as asdf-plugin is unix-centric based
        ...
        let asdf_plugin = config
            .asdf_plugin
            .unwrap_or_else(|| self.tool.name().to_string());
    ...
    pub fn install_plugin(&self, repository: &str) {
        ...
        let plugin_dir = asdf_dir.join("plugins").join(self.tool.name());
    ...
    pub fn pre_install(&self, mut input: InstallHook) {
        ...
        let repository = config
            .asdf_repository
            .unwrap_or_else(|| format!("https://github.com/asdf-vm/asdf-{}.git", self.tool.name()));
...
pub fn unpack_archive(Json(input): Json<UnpackArchiveInput>) -> FnResult<()> {
    ...
    if input_file.ends_with(".tar.xz") {
        ...
        return Err(
            PluginError::Other(format!("Unsupported archive format: {}", input_file)).into(),
        );
    }
