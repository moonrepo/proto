    ...
    pub fn inherit_builtin_plugins(&mut self) {
        ...
        if !self.plugins.contains_key("asdf") && is_allowed("asdf") {
            self.plugins.insert(
                Id::raw("asdf"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/asdf_backend-v0.1.0/asdf_backend.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("bun") && is_allowed("bun") {
            self.plugins.insert(
                Id::raw("bun"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/bun_tool-v0.15.0/bun_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("deno") && is_allowed("deno") {
            self.plugins.insert(
                Id::raw("deno"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/deno_tool-v0.15.0/deno_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("go") && is_allowed("go") {
            self.plugins.insert(
                Id::raw("go"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/go_tool-v0.16.0/go_tool.wasm".into()
                }))
            );
        }
        ...
        for depman in ["npm", "pnpm", "yarn"] {
            if !self.plugins.contains_key(depman) && is_allowed(depman) {
                self.plugins.insert(
                    Id::raw(depman),
                    PluginLocator::Url(Box::new(UrlLocator {
                        url: "https://github.com/moonrepo/plugins/releases/download/node_depman_tool-v0.15.0/node_depman_tool.wasm".into()
                    }))
                );
            }
        }
        ...
        if !self.plugins.contains_key("python") && is_allowed("python") {
            self.plugins.insert(
                Id::raw("python"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/python_tool-v0.14.0/python_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("uv") && is_allowed("uv") {
            self.plugins.insert(
                Id::raw("uv"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/python_uv_tool-v0.2.0/python_uv_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("ruby") && is_allowed("ruby") {
            self.plugins.insert(
                Id::raw("ruby"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/ruby_tool-v0.2.0/ruby_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key(SCHEMA_PLUGIN_KEY) {
            self.plugins.insert(
                ...
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/plugins/releases/download/schema_tool-v0.17.0/schema_tool.wasm".into()
                }))
            );
        }
