    ...
    pub fn inherit_builtin_plugins(&mut self) {
        ...
        if !self.plugins.contains_key("deno") {
            self.plugins.insert(
                Id::raw("deno"),
                PluginLocator::SourceUrl {
                    url: "https://github.com/moonrepo/deno-plugin/releases/download/v0.9.0/deno_plugin.wasm".into()
                }
            );
        }
        ...
        if !self.plugins.contains_key(SCHEMA_PLUGIN_KEY) {
            self.plugins.insert(
                ...
                PluginLocator::SourceUrl {
                    url: "https://github.com/moonrepo/schema-plugin/releases/download/v0.9.0/schema_plugin.wasm".into()
                }
            );
        }
    }
