    ...
    pub fn inherit_builtin_plugins(&mut self) {
        ...
        if !self.plugins.contains_key("python") {
            self.plugins.insert(
                Id::raw("python"),
                PluginLocator::SourceUrl {
                    url: "https://github.com/moonrepo/python-plugin/releases/download/v0.9.0/python_plugin.wasm".into()
                }
            );
        }
        ...
        if !self.plugins.contains_key(SCHEMA_PLUGIN_KEY) {
            self.plugins.insert(
                ...
                PluginLocator::SourceUrl {
                    url: "https://github.com/moonrepo/schema-plugin/releases/download/v0.11.0/schema_plugin.wasm".into()
                }
            );
        }
    }
