...
impl ProtoConfig {
    ...
    pub fn builtin_proto_plugin(&self) -> PluginLocator {
        PluginLocator::Url(Box::new(UrlLocator {
            url: "https://github.com/moonrepo/tools/releases/download/proto_tool-v0.2.0/proto_tool.wasm".into()
        }))
    }
    ...
    pub fn inherit_builtin_plugins(&mut self) {
        ...
        if !self.plugins.contains_key("go") && is_allowed("go") {
            self.plugins.insert(
                Id::raw("go"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/tools/releases/download/go_tool-v0.13.0/go_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("node") && is_allowed("node") {
            self.plugins.insert(
                Id::raw("node"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/tools/releases/download/node_tool-v0.12.1/node_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key("python") && is_allowed("python") {
            self.plugins.insert(
                Id::raw("python"),
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/tools/releases/download/python_tool-v0.11.0/python_tool.wasm".into()
                }))
            );
        }
        ...
        if !self.plugins.contains_key(SCHEMA_PLUGIN_KEY) {
            self.plugins.insert(
                ...
                PluginLocator::Url(Box::new(UrlLocator {
                    url: "https://github.com/moonrepo/tools/releases/download/schema_tool-v0.15.0/schema_tool.wasm".into()
                }))
            );
        }
