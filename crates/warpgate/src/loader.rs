...
impl PluginLoader {
    ...
    pub fn load_client(&self) -> miette::Result<&reqwest::Client> {
        self.http_client
    ...
    async fn download_plugin(
        ...
        download_from_url_to_file(source_url, &temp_file, self.load_client()?).await?;
    ...
    async fn download_plugin_from_github(
        ...
        let mut request = self.load_client()?.get(&api_url);
